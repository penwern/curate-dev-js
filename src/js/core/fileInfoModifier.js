...
const getQuarantineStatus = (scan, scan2, scanTag, scanDate) => {
  ...
  if (
    (!scanTag || scan == "File has not been scanned") &&
    openWs == "quarantine" &&
    scanTag !== "Scan Limit Exceeded"
  ) {
...
function addFileInfo(fileInfoPanel) {
  ...
  var scans = ["usermeta-virus-scan-first", "usermeta-virus-scan-second"].map(
    (item) => getMetaItem(item) || "File has not been scanned",
  );
...
const selectHandler = (e, fileInfoPanel) => {
  ...
  if (
    !e.memo._selectedNodes ||
    e.memo._selectedNodes.length == 0 ||
    e.memo._selectedNodes[0] == selectedNode
  ) {
    return;
  }
  ...
  if (fileInfoPanel) {
    ...
    fileInfoPanel.firstElementChild.addEventListener("click", () => {
      if (
        ...
        fileInfoPanel
          .querySelector('[class*="mdi-chevron-"]')
    ...
    if (mutation.type === "childList") {
      for (const node of mutation.addedNodes) {
        if (
          ...
          node.classList.contains("panelCard") &&
          node.innerText.includes("File Info")
        ) {
          processFileInfoPanel(node);
        } else if (
          node instanceof HTMLElement &&
          node.classList.contains("panelContent") &&
          node.parentElement.classList.contains("panelCard") &&
          node.parentElement.innerText.includes("File Info")
        ) {
