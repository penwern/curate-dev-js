    ...
    enforceWorkspaceUpload: {
      ...
      handler: (e) => {
        pydio.user.getIdmUser().then((idmUser) => {
          if (
            !["quarantine", "personal-files", "common files"].includes(
              Curate.workspaces.getOpenWorkspace(),
            ) &&
            !idmUser.Roles.find((r) => (r.Label = "Admin")) &&
            ...
            Curate.ui.modals
              .curatePopup({
                title: "You do not have permission to upload to this workspace",
                type: "warning",
                content: content,
              })
              .fire();
    ...
    enforceNoCustomActions: {
      ...
      handler: (e) => {
        ...
        if (window.location.pathname.includes("/public/")) {
          ...
          const darkModeButton = Array.from(document.querySelector("#main-toolbar").children).find(
            (n) => n.type === "button" && n.querySelector(".action-local_toggle_theme"),
          );
          const newButton = Array.from(document.querySelectorAll(".toolbars-button-menu")).find(
            (n) => n.classList.length == 1,
          );
          moreButton ? moreButton.remove() : null;
          darkModeButton ? darkModeButton.remove() : null;
          newButton ? newButton.remove() : null;
