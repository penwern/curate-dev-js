    ...
    while (cur) {
      ...
      if (name === "zp") {
        ...
        return {
          ...
          identifier:
            props.namespace && props.componentName
              ? `${props.namespace}.${props.componentName}`
              : null,
        };
      }
...
function useHeaderControls(markerRef, open, setOpen, label, pinState) {
  ...
  React.useLayoutEffect(() => {
    ...
    const createButton = (kind, iconAttr) => {
      ...
      if (!button) {
        ...
        icon.style.cssText =
          "color: inherit; cursor: inherit; position: relative; font-size: 18px; display: inline-block; user-select: none; transition: 450ms cubic-bezier(0.23, 1, 0.32, 1);";
    ...
    const onHeaderClick = (event) => {
      ...
      if (pinParts.wrapper.contains(event.target) || toggleParts.wrapper.contains(event.target))
        return;
      const { currentPin, identifier, setColumnPin } = latestRef.current;
      if (currentPin && currentPin !== identifier) {
        setOpen(true);
  ...
  React.useLayoutEffect(() => {
    ...
    if (pinButton) {
      ...
      const pinLabel = isPinnedSelf
        ? `Unpin ${label}`
        : hasOtherPin
          ? `Pin ${label} instead`
          : `Attach card (full height)`;
