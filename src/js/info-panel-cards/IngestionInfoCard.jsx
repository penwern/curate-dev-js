...
const STATUS_CFG = {
  ...
  Quarantined: {
    label: "Quarantined",
    sub: "In active quarantine period",
    color: "#FB8C00",
...
function getStatus(scanTag, openWs, scan1, scan2) {
  if (!scanTag) {
    return openWs === "quarantine"
      ? {
          label: "Unscanned",
          sub: "Awaiting virus scan",
          color: "#E53935",
          icon: "mdi-shield-alert-outline",
        }
      : {
          label: "Unscanned",
          sub: "Reupload to Quarantine to be scanned",
          color: "#E53935",
          icon: "mdi-shield-alert-outline",
        };
  }
  ...
  if ((scanTag === "Passed" || scanTag === "QuarantineTag") && scan1?.passed && !scan2) {
    return openWs === "quarantine"
      ? {
          label: "Quarantined",
          sub: "First scan passed - awaiting second scan",
          color: "#42A5F5",
          icon: "mdi-shield-lock-outline",
        }
      : {
          label: "At Risk",
          sub: "First scan passed - move to Quarantine for second scan",
          color: "#FB8C00",
          icon: "mdi-shield-alert",
        };
  }
...
function ScanChip({ result }) {
  ...
  return (
    ...
    >
      <i
        className={`mdi ${result.passed ? "mdi-check" : result.failed ? "mdi-close" : "mdi-minus"}`}
        style={{ fontSize: 12 }}
      />
        ...
        {effectiveOpen && (
          <>
            ...
            >
              ...
              <div>
                <div
                  style={{ fontSize: 13, fontWeight: 700, color: status.color, lineHeight: 1.2 }}
                >
                  {status.label}
                </div>
                {status.sub && (
                  <div
                    ...
                    style={{ fontSize: 13, marginTop: 2, lineHeight: 1.3 }}
                  >
                    {status.sub}
                  </div>
                )}
            ...
            <div style={{ padding: "0 12px", display: "flex", flexDirection: "column", gap: 3 }}>
              {[
                ["First", scan1, scan1?.date],
                ["Second", scan2, scan2?.date],
              ].map(([label, result, date]) => (
                <div
                  key={label}
                  className="curate-ing-scanrow"
                  style={{
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  <span className="infoPanelLabel" style={{ fontSize: 13, minWidth: 44 }}>
                    {label}
                  </span>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    {date && (
                      <span
                        className="infoPanelLabel"
                        style={{ fontSize: 12, fontFamily: "monospace", opacity: 0.7 }}
                      >
                        {date}
                      </span>
                    )}
            ...
            <div style={{ padding: "0 12px", display: "flex", flexDirection: "column", gap: 3 }}>
              {sfMatch?.format && (
                ...
                >
                  <span className="infoPanelLabel" style={{ fontSize: 13, flexShrink: 0 }}>
                    Format
                  </span>
                  <span
                    className="infoPanelValue"
                    title={sfMatch.format}
                    style={{
                      fontSize: 13,
                      overflow: "hidden",
              ...
              {pronId ? (
                <div
                  ...
                  onClick={() =>
                    window.open(`https://www.nationalarchives.gov.uk/pronom/${pronId}`)
                  }
                ...
                >
                  ...
                  <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                    ...
                    <i
                      ...
                      style={{ fontSize: 12, color: "#42A5F5", opacity: 0.65 }}
                    />
                  </span>
                </div>
              ) : (
                ...
                >
                  <span className="infoPanelLabel" style={{ fontSize: 13 }}>
                    PRONOM
                  </span>
                  <span className="infoPanelValue" style={{ fontSize: 13, fontStyle: "italic" }}>
                    Not characterised
                  </span>
                </div>
              )}
              ...
              {basisLabel && (
                ...
                >
                  ...
                  <span className="infoPanelValue" style={{ fontSize: 13, textAlign: "right" }}>
                    {basisLabel}
                  </span>
                </div>
              )}
              ...
              {sf?.scandate && (
                ...
                >
                  ...
                  <span className="infoPanelValue" style={{ fontSize: 13 }}>
                    {new Date(sf.scandate).toLocaleDateString(undefined, {
                      day: "2-digit",
                      month: "short",
                      year: "numeric",
                    })}
              ...
              {sf?.identifiers?.[0]?.details && (
                ...
                >
                  ...
                  >
                    {sf.identifiers[0].details.split("; ").map((s, i) => (
              ...
              {sfMatch?.warning && (
                <>
                  ...
                  >
                    ...
                    <div>
                      <div
                        style={{ fontSize: 13, fontWeight: 700, color: "#FB8C00", lineHeight: 1.2 }}
                      >
                        {sfMatch.warning}
                      </div>
                      {warningExplanation && (
              ...
              {sfFile?.errors && (
                ...
                >
                  ...
                  />
                  <div className="infoPanelLabel" style={{ fontSize: 13, lineHeight: 1.3 }}>
                    {sfFile.errors}
                  </div>
                </div>
              )}
