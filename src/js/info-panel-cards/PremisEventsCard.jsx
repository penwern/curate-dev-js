...
function processEvents(premis) {
  ...
  while (i < premis.length) {
    ...
    if (ev.event_type === "Metadata Modification") {
      let count = 1;
      while (i + count < premis.length && premis[i + count].event_type === "Metadata Modification")
        count++;
...
function getScrollParent(el) {
  ...
  return (
    document.querySelector("#info_panel .scrollarea-content") ||
    document.querySelector("#info_panel")
  );
}
...
function PremisEvent({ event, isLast, isLatest, idx }) {
  ...
  const outcomeNote =
    event.event_outcome_information?.event_outcome_detail?.event_outcome_detail_note || "";
            ...
            <div className="curate-premis-detail-inner">
              <div style={{ paddingTop: 4, paddingBottom: 2 }}>
                ...
                <div
                  ...
                  style={{ fontSize: 10, fontFamily: "monospace", marginTop: 2 }}
                >
...
function PremisEventsCard(props) {
  ...
  const showMore = () =>
    setVisibleCount((current) => Math.min(events.length, current + LOAD_MORE_STEP));
  const showFewer = () => {
              ...
              <div className="curate-premis-controls">
                ...
                >
                  ...
                  <span className="curate-premis-load-more-label">
                    Show {Math.min(LOAD_MORE_STEP, hiddenCount)} earlier event
                    {Math.min(LOAD_MORE_STEP, hiddenCount) === 1 ? "" : "s"}
                  </span>
                </button>
                <span className="curate-premis-hidden-count">{hiddenCount} hidden</span>
              </div>
            )}
            ...
            {canCollapse && (
              <div className="curate-premis-controls bottom">
                ...
                >
                  ...
                  <span className="curate-premis-load-more-label">Show fewer</span>
                </button>
              </div>
            )}
          </div>
