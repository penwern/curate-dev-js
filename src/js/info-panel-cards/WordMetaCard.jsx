...
function WordMetaCard(props) {
  ...
  const activeFlags = [];
  if (flags.has_macros)
    activeFlags.push({ icon: "mdi-code-braces", label: "Macros", state: "danger" });
  if (flags.document_protection_enabled)
    activeFlags.push({ icon: "mdi-lock-outline", label: "Protected", state: "warn" });
  if (flags.track_revisions_enabled)
    activeFlags.push({ icon: "mdi-track-changes", label: "Track Changes", state: "warn" });
  if (flags.is_template)
    activeFlags.push({ icon: "mdi-file-outline", label: "Template", state: "info" });
  if (flags.has_comments_part)
    activeFlags.push({
      icon: "mdi-comment-multiple-outline",
      label: "Has Comments",
      state: "info",
    });
  if (flags.has_custom_properties)
    activeFlags.push({ icon: "mdi-tag-multiple-outline", label: "Custom Props", state: "info" });
  if (flags.has_footnotes_part)
    activeFlags.push({ icon: "mdi-format-superscript", label: "Footnotes", state: "neutral" });
  if (flags.has_endnotes_part)
    activeFlags.push({ icon: "mdi-format-footnote", label: "Endnotes", state: "neutral" });
        ...
        {effectiveOpen && (
          <>
            ...
            {showAuthorship && (
              ...
              >
                <div
                  style={{ padding: "9px 12px", display: "flex", flexDirection: "column", gap: 5 }}
                >
                  {author && (
                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                      <i
                        className="mdi mdi-account-outline"
                        style={{ fontSize: 13, color: INDIGO, opacity: 0.7, flexShrink: 0 }}
                      />
                      <span
                        style={{
                          fontSize: 12,
                  ...
                  {lastModBy && lastModBy !== author && (
                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                      <i
                        ...
                        style={{ fontSize: 13, color: INDIGO, opacity: 0.55, flexShrink: 0 }}
                      />
                      <span
                        style={{
                          fontSize: 12,
                    ...
                    >
                      {created && (
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          <i
                            className="mdi mdi-calendar-plus-outline"
                            style={{ fontSize: 11, color: "#90A4AE" }}
                          />
                      ...
                      {modified && modified !== created && (
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          <i
                            className="mdi mdi-calendar-edit-outline"
                            style={{ fontSize: 11, color: "#90A4AE" }}
                          />
                  ...
                  {appName && (
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
                        <i
                          className="mdi mdi-application-outline"
                          style={{ fontSize: 13, color: "#78909C", flexShrink: 0 }}
                        />
                        <span
                          style={{
                            fontSize: 12,
                    ...
                    >
                      {editMinutes != null && (
                        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                          <i
                            className="mdi mdi-timer-outline"
                      ...
                      {template && (
                        <div
                          style={{
                            ...
                            alignItems: "center",
                            gap: 5,
                            marginLeft: editMinutes != null ? "auto" : 0,
                          }}
                        >
                          <i
