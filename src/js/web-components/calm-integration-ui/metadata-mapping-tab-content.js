...
class MetadataMappingTabContent extends LitElement {
  ...
  async _loadWhitelistData() {
    ...
    try {
      ...
      const response = await this.apiService.getWhitelist();
      ...
      if (response) {
        ...
        let normalizedFields = rawFields.map((field) => {
          ...
          return {
            whitelist_id: field.whitelist_id ?? field.id ?? null,
        ...
        if (!hasExplicitDefault) {
          if (defaultCount > 0) {
            ...
            normalizedFields = normalizedFields.map((field) => ({
              ...field,
              is_default: defaultSet.has(field.field_name),
            }));
            defaultCount = defaultSet.size;
          } else {
            normalizedFields = normalizedFields.map((field) => ({
              ...field,
              is_default: field.is_default === true,
            }));
            defaultCount = normalizedFields.filter((field) => field.is_default).length;
          }
        } else {
          normalizedFields = normalizedFields.map((field) => ({
            ...field,
        ...
        this._whitelistData = {
          ...
          default_fields: defaultCount,
          custom_fields: customCount,
        };
      } else {
        this._whitelistData = null;
      }
  ...
  async _handleDiscoverFields() {
    ...
    try {
      ...
      this._discoveryMessage = "Collecting CALM metadata schema...";
      ...
      if (result.status === "success") {
        ...
        this._discoveryMessage =
          result.message ||
          `Discovered ${total} CALM fields (${whitelisted} whitelisted, ${nonWhitelisted} not whitelisted)`;
          ...
          <div class="action-section">
            ...
            >
              <span style="display: flex; align-items: center; gap: 8px;"
                >${searchIcon}${
                  this._isDiscoveringFields ? "Discovering..." : "Discover Fields"
                }</span
              >
            </md-filled-button>
            ${when(
        ...
        ${when(
          ...
          () => html`
            <div class="action-section">
              ...
              >
                <span style="display: flex; align-items: center; gap: 8px;">
                  ${hasUnmappedFields ? blockIcon : cachedIcon}${
              ...
              <span
                class="cached-badge ${
                  this._isProcessingCached ? "processing" : hasUnmappedFields ? "blocked" : ""
                }"
              >
                <span style="display: flex; align-items: center; gap: 8px;">
                  ${when(
                    ...
                    action === "mapped" && mappings.length > 0
                      ? html`
                          <span class="mapped-to-label"
                            ...
                            field${
                              mappings.filter((m) => m.action === "mapped" && m.curate_field)
                                .length > 1
                                ? "s"
                                : ""
                            }:</span
          ...
          <md-dialog open @closed=${this._closeMappingDialog}>
            ...
            <div slot="actions">
              ...
              <md-filled-button
                ...
                ?disabled=${
                  this._currentDialogAction === "mapped" && this._selectedCurateFields.size === 0
                }
              >
                ${
                  this._currentDialogAction === "mapped"
