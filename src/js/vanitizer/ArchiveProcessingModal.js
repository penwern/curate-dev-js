...
(function archiveProcessingModalEntry() {
  const API_BASE_URL =
    window.CURATE_EMAIL_API_BASE_URL ||
    window.CURATE_EMAIL_API_BASE ||
    "http://localhost:8001";
    ...
    Curate.ui.modals.curatePopup(
      {
        ...
        message: "Select an archive to process before launching this action.",
        buttonType: "close",
      },
      {}
    ).fire();
  ...
  const templateContent = `
    <archive-processing-status data-role="status-card"></archive-processing-status>
  `;
  ...
  async function requestJobStatus(jobId) {
    return Curate.api.fetchCurate(
      `/api/archives/status/${encodeURIComponent(jobId)}`,
      "GET",
      undefined,
      API_BASE_URL
    );
