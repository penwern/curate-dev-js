function createCuratePopup(title, inputs) {
  ...
  optionsContainer.style =
    "display: flex;align-items: center;flex-wrap: nowrap;flex-direction: column;";
  ...
  saveConfig.addEventListener("click", (e) => {
    ...
    inputIds.forEach((id) => {
      ...
      if (input.type == "submit") {
        //do not add "go to atom config" button
        return;
      }
      if (input.disabled) {
        ...
        if (r) {
          ...
          getPreservationConfigs().then((r) => {
            ...
            if (curConfig.id) {
              ...
              createConfigsBox(savedScrollContainer, savedConfigsContainer, [
                newConfigs.find((obj) => obj.id === curConfig.id),
              ]);
            } else {
              const newObject = newConfigs.find(
                (newObj) => !curConfigs.some((curObj) => curObj.id === newObj.id),
              );
...
function createConfigsBox(target, container, configs) {
  ...
  configs?.forEach((config) => {
    ...
    configDelete.addEventListener("mouseleave", (e) => {
      ...
      if (
        e.toElement == configItem ||
        e.toElement == configItem.querySelector(".saved-config-information")
      ) {
        configItem.style.backgroundColor = "var(--md-sys-color-outline-variant)";
      } else {
        configItem.style.backgroundColor = "var(--md-sys-color-on-secondary)";
...
const inputs = [
  ...
  },
  {
    ...
    inputs: [
      {
        label: "Normalise Objects",
        name: "normalize",
        type: "toggle",
        suboptions: [
          {
            label: "Image Normalisation Format",
            name: "image_normalization_tiff",
            type: "dropdown",
            options: ["TIFF", "JPEG2000"],
          },
        ],
      },
    ],
  },
  {
    ...
    inputs: [
      {
        label: "Create Dissemination Package",
        name: "dip_enabled",
        type: "toggle",
        suboptions: [
          {
            label: "Dissemination Information",
            name: "dip_info",
            type: "info",
            text: "Create dissemination packages from AIPs generated by this config. Created DIPs will automatically be connected to the linked description of the source data. For this option to work, you must configure a connected AtoM instance.",
          },
          {
            label: "Go to AtoM Configuration",
            name: "atom_config",
            type: "button",
                ...
                { title: readonly ? "AtoM Configuration" : "Connect to Your AtoM Instance" },
                {
                  afterLoaded: (c) => {
                    const t = document.createElement(
                      readonly ? "atom-config-readonly" : "connect-to-atom",
                    );
  ...
  {
    category: "Packaging and Compression",
    inputs: [
      {
        label: "AIP Packaging Type",
        name: "process_type",
        type: "dropdown",
        options: ["standard", "eark"],
      },
      {
        label: "Compress AIPs",
        name: "compress_aip",
        ...
        suboptions: [
          {
            ...
            type: "info",
            text: "Compressing AIPs will make their contents unsearchable and prevent descriptive metadata from being reassociated with output objects. You can compress your AIPs for distribution or deep-storage while conserving the uncompressed AIP by right-clicking an AIP in a workspace.",
          },
          {
            label: "Compression Algorithm",
            name: "compression_algorithm",
            type: "dropdown",
            options: ["tar", "tar_bzip2", "tar_gzip", "s7_copy ", "s7_bzip2", "s7_lzma"],
          },
          {
    ...
    inputs: [
      {
        ...
        name: "gen_transfer_struct_report",
        type: "toggle",
      },
